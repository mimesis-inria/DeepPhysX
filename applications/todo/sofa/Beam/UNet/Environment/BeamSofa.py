...
class BeamSofa(SofaEnvironment):
    ...
    def createFEM(self):
        ...
        # Material
        self.root.fem.addObject('NeoHookeanMaterial', name='NH', young_modulus=2500, poisson_ratio=0.45)
    ...
    def onAnimateBeginEvent(self, event):
        ...
        # Define random box
        side = randint(0, 11)
        x_min = p_grid.min[0] if side in (3, 4) else randint(p_grid.min[0], p_grid.max[0] - 10)
        x_max = p_grid.max[0] if side in (5, 6) else randint(x_min + 10, p_grid.max[0] + 1)
        y_min = p_grid.min[1] if side in (7, 8) else randint(p_grid.min[1], p_grid.max[1] - 10)
        y_max = p_grid.max[1] if side in (9, 10) else randint(y_min + 10, p_grid.max[1] + 1)
        z_min = p_grid.min[2] if side == 0 else randint(p_grid.min[2], p_grid.max[2] - 10)
        z_max = p_grid.max[2] if side in (1, 2, 3) else randint(z_min + 10, p_grid.max[2] + 1)
        ...
        K = randint(10 * (self.K - 1), 10 * self.K)
        F = K * (F / norm(F))
