...
# Create a networks as a BaseNetwork child class
class DummyNetwork(BaseNetwork):

    def __init__(self, config):

        BaseNetwork.__init__(self, config)
        # There is no networks architecture to define in our DummyNetwork
    ...
    This DummyNetwork has no parameters so it does not behave like a real neural networks.
    """
...
class DummyTransformation(BaseTransformation):
    ...
    def transform_before_prediction(self, data_net):
        ...
        # Do not transform the networks data
        return data_net
